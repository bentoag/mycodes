...
public class MyProfile {
	...
	public static void main(String[] args) {
		...
		System.out.println("**** BEM VINDO AO SISTEMA DE CADASTRO DE PERFIL MÉDICO: ****");
		...
		if(opcao == 1) {
			...
			System.out.printf("%s %s %s %n%s %d %s","Bem vindo : ", user.getNome(), user.getSobrenome(), "Você tem:  ", user.calculaIdade(), " anos.");
			System.out.printf("%n%s %s","Sua data de nascimento é:  ", user.getDataNascimento());
			System.out.printf("%n%s %d %s", "Sua Frequência Cardíaca Máxima deve ser: ", user.frequenciaCardioMax(), " bt/min");
			System.out.printf("%n%s %s", "Sua Frequência Cardíaca Ideal ", user.frequenciaAlvo());
			
			//validando o IMC
			if(user.getIMC() == -1) {
				System.out.println();
				System.out.println("Parâmetros inválidos, não podemos calcular seu IMC!");
			}else {
				System.out.printf("%n%s %.2f %n", "Seu IMC é igual a: ", user.getIMC());
